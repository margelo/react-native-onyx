...
const provider = {
     ...
     * @return {Promise<void>}
     */
    setItem: (key, value) => console.log("setItem", key, value) || set(key, value, getCustomStore()),
     ...
     */
    multiGet: (keysParam) => console.log("multiGet", keysParam) || getMany(keysParam, getCustomStore()).then((values) => _.map(values, (value, index) => [keysParam[index], value])),
     ...
     * @return {Promise<void>}
     */
    multiMerge: (pairs) => console.log("multiMerge", pairs) ||
    ...
    mergeItem(key, _changes, modifiedData) {
        console.log("mergeItem", key, _changes, modifiedData);
     ...
     * @return {Promise<void>}
     */
    multiSet: (pairs) => console.log("multiSet", pairs) || setMany(pairs, getCustomStore()),

    /**
     * Clear everything from storage and also stops the SyncQueue from adding anything more to storage
     * @returns {Promise<void>}
     */
    clear: () => console.log("clear") || clear(getCustomStore()),
     ...
     */
    getAllKeys: async () => { const key = await keys(getCustomStore()); console.log("getAllKeys", key); return key },
     ...
     * @return {Promise<*>}
     */
    getItem: (key) => console.log("getItem", key) || 
     ...
     * @returns {Promise<void>}
     */
    removeItem: (key) => console.log("removeItem", key) ||  del(key, getCustomStore()),
     ...
     * @returns {Promise}
     */
    removeItems: (keysParam) => console.log("removeItems", keysParam) || delMany(keysParam, getCustomStore()),
