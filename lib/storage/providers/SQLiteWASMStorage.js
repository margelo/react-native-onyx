...
const provider = {
     ...
     * @return {Promise<void>}
     */
    multiMerge: (pairs) => log("multiMerge", {pairs}) || Promise.resolve(),
    ...
    mergeItem(key, _changes, modifiedData) {
        log("mergeItem", key, _changes, modifiedData);
        return Promise.resolve();
    },
     ...
     */
    clear: () => log("clear") || Promise.resolve(db.exec({sql: 'DELETE FROM keyvaluepairs;'})),
     ...
     * @returns {Promise<void>}
     */
    removeItem: (key) => log("removeItem", key) || Promise.resolve(db.exec({sql: 'DELETE FROM keyvaluepairs WHERE record_key = ?;', bind: [key]})),
