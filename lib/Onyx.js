...
const VALID_UPDATE_METHODS = [METHOD.CLEAR, METHOD.SET, METHOD.MERGE, METHOD.MERGE_COLLECTION, METHOD.MULTI_SET];
...
function update(data) {
    // Validate the Onyx object is in the format we expect
    _.each(data, ({onyxMethod, key, value}) => {
        if (!_.contains(VALID_UPDATE_METHODS, onyxMethod)) {
    ...
    const mergeMap = data
        .filter(({onyxMethod}) => onyxMethod === METHOD.MERGE)
    ...
    // merge the onyx.merges into single change set per key
    // eslint-disable-next-line rulesdir/prefer-underscore-method
    const batchedMerges = Object
        .entries(mergeMap)
        .map(([key, values]) => {
            const finalValue = applyMerge(undefined, values, false);
            return {onyxMethod: METHOD.MERGE, key, value: finalValue};
        });

    // remove individual merges and replace them with the batched versions
    // eslint-disable-next-line rulesdir/prefer-underscore-method
    const finalData = data

        // remove individual merges
        .filter(({onyxMethod}) => onyxMethod !== METHOD.MERGE)

        // Add batched merges
        .concat(batchedMerges)

        // Put clear operation on top
        .sort(a => (a.onyxMethod === METHOD.CLEAR ? -1 : 1));

    return innerUpdate(finalData);
}
